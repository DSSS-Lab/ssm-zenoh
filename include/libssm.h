...
extern "C"
{
	...
	typedef struct
	{
		...
		int sem_lock_id;                        ///< セマフォー用のsemid
		int sem_cond_id;                        ///< セマフォーの条件付きロックのsemid
		int sem_lock_key;
		int sem_cond_key;
	...
	/* shm functions */
	void shm_init_header( ssm_header * header, int ssize, int hsize, ssmTimeT cycle, int sem_key );
