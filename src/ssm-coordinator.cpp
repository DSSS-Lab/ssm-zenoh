...
int alloc_ssm_block( int ssize, int hsize, ssmTimeT cycle, char **shm_h, int *offset, int node)
{
	int s_id;
	...
	*offset = 0;
	...
	/* ssm_header 初期化 */
	shm_init_header( ( ssm_header * ) * shm_h, ssize, hsize, cycle, node );
...
SSM_List *add_ssm_list( int node, char *name, int suid, int ssize, int hsize, ssmTimeT cycle )
{
	...
	p->shm_id = alloc_ssm_block( ssize, hsize, cycle, &( p->shm_ptr ), &( p->shm_offset ), node);
	if( !p->shm_id )
		return 0;
...
int msq_loop( void )
{
	...
	while( 1 )
	{
		...
		case MC_CREATE:						/* センサのメモリ確保 */
			...
			if( verbosity_mode >= 2 )
				printf( "   |   :received shm_id:%d offset:%ld\n", msg.suid, msg.ssize );
			break;
